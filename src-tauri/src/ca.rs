...
pub fn proxy_certs_dir() -> PathBuf {
    if let Ok(xdg) = std::env::var("XDG_RUNTIME_DIR") {
        PathBuf::from(xdg).join("tillandsias").join("proxy-certs")
    } else {
        std::env::temp_dir().join("tillandsias-proxy-certs")
    }
}
...
pub fn generate_ephemeral_certs() -> Result<PathBuf, String> {
    ...
    let mut root_params =
        CertificateParams::new(Vec::<String>::new()).map_err(|e| format!("Root CA params: {e}"))?;
    root_params
        .distinguished_name
        .push(DnType::OrganizationName, "Tillandsias");
    root_params
        .distinguished_name
        .push(DnType::CommonName, "Tillandsias Ephemeral Root CA");
    ...
    fs::write(dir.join("root.crt"), &root_cert_pem).map_err(|e| format!("Write root.crt: {e}"))?;
