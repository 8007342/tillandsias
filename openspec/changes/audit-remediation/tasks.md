...
- [ ] 3.2 Remove or demote tray icon state switching to "future" (W2/F1 — SVGs exist but icon is static)
- [ ] 3.3 Remove Start/Rebuild menu actions from artifact-detection spec (F3 — never implemented)
- [ ] 3.4 Clarify destroy UX in app-lifecycle: server-side delay, not hold-button (W5)

## 4. Wiring (non-trivial code changes)

- [ ] 4.1 Wire Tauri updater to show "Update available (vX.Y.Z)" in tray menu (W7)
- [ ] 4.2 Extract detect_host_os() to shared module (O5 — duplicated in handlers.rs and runner.rs)

## 5. Verification

- [ ] 5.1 cargo clippy + fmt clean
