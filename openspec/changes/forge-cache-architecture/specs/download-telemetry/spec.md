...
- `cheatsheets/runtime/forge-paths-ephemeral-vs-persistent.md` — defines what "anti-pattern" means for which path
- `cheatsheets/runtime/forge-shared-cache-via-nix.md` — explains why bytes should land in per-project or shared cache, never in workspace
- The user's "feedback_zero_runtime_downloads_metric.md" memory rule (host Claude memory; not on disk in the repo)
