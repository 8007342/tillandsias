# Spec delta — appimage-build-pipeline
...
- `cheatsheets/utils/tar.md` — to be added in this change's tasks
  (currently absent); covers tar's `--exclude` semantics, streaming
  pipe pattern, and exit-code conventions.
- `cheatsheets/build/cargo.md` — covers cargo's read-only-source
  rejection and why `target/` cannot be shared across glibc versions.
- `cheatsheets/runtime/runtime-limitations.md` — covers the podman
  bind-mount RO/RW interaction model.
