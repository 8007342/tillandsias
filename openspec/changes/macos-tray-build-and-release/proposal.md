...
- **User-facing install UX (v0.0.1, ad-hoc signed)**: `curl -fsSL https://github.com/8007342/tillandsias/releases/latest/download/install-macos.sh | bash` puts the .app in /Applications/ and launches it cleanly (curl downloads are never quarantined, so Gatekeeper does not fire). The right-click-Open hint is printed only when the installed bundle actually carries `com.apple.quarantine` (order 421). Developer ID notarization comes in a post-v0.0.1 change.
