# host-browser-mcp delta — host-browser-mcp
...
- `cheatsheets/web/http.md` — URL parsing rules (scheme, host,
  userinfo, port) the allowlist enforces.
- `cheatsheets/web/cdp.md` — CDP method matrix this server uses;
  pinned to stable 1.3 per Chromium version. (NEW, this change.)
- `cheatsheets/web/mcp.md` — MCP JSON-RPC framing,
  initialize / tools/list / tools/call shapes; pinned to protocol
  revision `2025-06-18`. (NEW, this change.)
- `cheatsheets/runtime/forge-paths-ephemeral-vs-persistent.md` —
  ephemeral `--user-data-dir` lives in
  `$XDG_RUNTIME_DIR/tillandsias/mcp/`; the control socket lives under
  the same XDG runtime dir.
- `cheatsheets/runtime/networking.md` — Unix-domain socket peer
  credentials (`SO_PEERCRED`) underpin the authorisation requirement.
- `cheatsheets/architecture/event-driven-basics.md` — per-session
  tokio task with a shared dispatcher pattern, mirroring the
  control-socket design.
- `cheatsheets/security/owasp-top-10-2021.md` — A05 Security
  Misconfiguration and A07 Auth & Identification informed the
  allowlist + peer-credential design.
