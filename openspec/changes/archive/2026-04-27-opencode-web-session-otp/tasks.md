...
- [x] 1.1 Create `src-tauri/src/otp.rs` with public functions:
...
- [x] 1.2 Add `@trace spec:opencode-web-session-otp, spec:secrets-management`
  doc-comment headers on every public item.
- [x] 1.3 Unit tests in `src-tauri/src/otp.rs::tests`:
...
- [x] 2.1 Depends on `tray-host-control-socket` capability landing first.
- [x] 2.2 Add `IssueWebSession { project_label: String, cookie_value: [u8; 32] }`
...
- [x] 2.3 Tray-side sender: `src-tauri/src/otp.rs::issue_to_router(project_label,
...
- [x] 2.4 Router-side handler: in the router sidecar (introduced as part of
...
- [x] 2.5 Add `@trace spec:opencode-web-session-otp, spec:tray-host-control-socket`
  on every code path involved.

## 3. Router: dynamic.Caddyfile rewrite for `/_auth/login` + cookie matcher

- [x] 3.1 Modify `src-tauri/src/handlers.rs::regenerate_router_caddyfile` so the
...
- [x] 3.2 Add a per-project Caddyfile var holding the project's session-list
...
- [x] 3.3 Update `images/router/base.Caddyfile` to wire the sidecar admin
  endpoint and to declare the `@hassession` matcher behaviour at the
  global block level.
- [x] 3.4 Update `images/router/entrypoint.sh` to launch the router sidecar
  alongside `caddy run`; both processes share lifetime.

## 4. Router: per-project session_tokens HashMap

- [x] 4.1 Implement the router-side state structure:
  `Mutex<HashMap<String, Vec<SessionEntry>>>` where
  `SessionEntry { value: [u8; 32], state: Pending(deadline) | Active }`.
- [x] 4.2 Implement `push(project_label, cookie_value)` — append a new
  `Pending` entry with `deadline = now + 60s`.
- [x] 4.3 Implement `validate(project_label, cookie_value) -> bool` — match
  against any entry, transition `Pending` → `Active` on first match, return
  `true`.
- [x] 4.4 Implement `evict_expired()` — periodic task (1s tick) removes
  `Pending` entries past their deadline.
- [x] 4.5 Implement `evict_project(project_label)` called when the project's
  container stack stops.
- [x] 4.6 Audit log every push, validate (success + failure), and evict event
  with `category = "router"`, `spec = "opencode-web-session-otp"`, value
  redacted.

## 5. Tray: CDP client integration to set cookie before navigation

- [x] 5.1 Depends on `host-chromium-on-demand` capability landing first
  (provides the bundled Chromium binary launched with `--remote-debugging-port=<random>`).
- [x] 5.2 Add a minimal CDP client (either `headless_chrome` crate or a small
  hand-rolled JSON-RPC client over the WebSocket exposed by Chromium) under
  `src-tauri/src/cdp.rs`.
- [x] 5.3 Implement `attach_and_set_cookie(cdp_port: u16, target_url: &str,
...
- [x] 5.4 Unit tests with a mocked CDP server (httpmock or similar) covering:
...
- [x] 6.1 Modify `src-tauri/src/browser.rs::launch_for_project` to:
...
- [x] 6.2 Update both call sites in `src-tauri/src/handlers.rs` (fresh attach
  and reattach branches) — both must trigger a fresh OTP issue + cookie
  injection.
- [x] 6.3 Update browser detection: bundled Chromium becomes the only path
...
- [x] 7.1 Tray unit tests (covered in 1.3 + 5.4 above).
- [x] 7.2 Router unit tests in the sidecar:
...
- [x] 7.3 Integration test (host-side, requires podman):
...
- [x] 8.1 Create `cheatsheets/web/cookie-auth-best-practices.md` with:
...
- [x] 8.2 Update `docs/cheatsheets/secrets-management.md` to add the OTP +
  session cookie row to the "Secret types" table with a note pointing at
  this change's spec deltas.

## 9. Versioning

- [x] 9.1 After `/opsx:archive`, run `./scripts/bump-version.sh --bump-changes`.
- [x] 9.2 Commit with the trace URL footer per the project convention.
