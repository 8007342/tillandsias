# init-command Delta Specification

## Purpose
Update to init-command spec to support `--debug` flag and incremental rebuild behavior.
...
- **THEN** all images are built in sequence (proxy, forge, git, inference), progress is shown on stdout, and the command exits with code 0
