...
- [x] 3.1 After successful build, add `<Home> ~/src >` if projects exist
- [x] 3.2 After successful build + GitHub auth, add `<Cloud> Cloud >` for remote projects
- [x] 3.3 If no GitHub credentials, show `<Key> GitHub login` instead of Cloud
- [x] 3.4 Hide Projects/Cloud items until environment is fully ready
- [x] 3.5 Add `@trace spec:tray-minimal-ux` annotations to conditional menu logic

## 4. Project Click Behavior

- [x] 4.1 Modify project click handler to launch OpenCode Web container
- [x] 4.2 Add logic to clone remote project to local before launching (if not cloned)
- [x] 4.3 Once container is healthy, launch safe browser in `tillandsias-chromium-core` container
- [x] 4.4 Use `tillandsias-browser-tool` to communicate between tray and browser container
- [x] 4.5 Add `@trace spec:tray-minimal-ux` annotations to project click handler

## 5. Stale Container Cleanup

- [x] 5.1 Add startup routine to clean up stale `tillandsias-*` containers
- [x] 5.2 Remove stopped/orphaned containers not tracked in TrayState
- [x] 5.3 Allow new containers to regenerate without conflicts
- [x] 5.4 Add `@trace spec:tray-minimal-ux` annotation to cleanup logic
