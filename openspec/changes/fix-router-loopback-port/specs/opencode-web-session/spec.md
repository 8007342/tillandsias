...
listens on TCP port `8080` because rootless podman cannot bind to ports below
`net.ipv4.ip_unprivileged_port_start` (default `1024` on Fedora and most distros).
...
listener inside the router stays on port `:80` (which is allowed inside
the container's user namespace); only the host-side publish moves to
`8080`.
...
#### Scenario: URL is hostname-with-port — no encoded path
- **WHEN** the tray constructs the launch URL for a project named
  `thinking-service`
- **THEN** the URL is exactly
  `http://thinking-service.opencode.localhost:8080/`
...
- **WHEN** the browser loads `http://<project>.opencode.localhost:8080/`
- **THEN** `window.isSecureContext` returns `true` (per W3C Secure
  Contexts §3.1; loopback origins are secure regardless of port)
...
- `cheatsheets/runtime/networking.md` — confirms in-enclave host-naming and proxy egress rules.
- `cheatsheets/runtime/forge-container.md` — documents that the forge user (UID 1000) cannot bind privileged ports; same applies to the router container's host publish (the publish itself happens in podman's rootless networking layer).
