...
Every path inside a forge container is classified as HOT (RAM-backed tmpfs) or
COLD (disk-backed overlayfs / host bind-mount). The HOT tier is reserved for
agent-readable knowledge that is re-read on every prompt: project source code
and cheatsheets. All other paths stay on the COLD tier. A pre-flight RAM check
refuses to launch when the host cannot satisfy the aggregate tmpfs budget with a
1.25× headroom factor.
...
- `cheatsheets/runtime/forge-hot-cold-split.md` — primary reference for HOT vs COLD classification, budget computation, and tmpfs mount table
- `cheatsheets/runtime/forge-paths-ephemeral-vs-persistent.md` — path taxonomy with backing-store column
- `cheatsheets/runtime/forge-container.md` — forge container overview and launch model
