...
- **ADDED (D6 amendment 2026-05-25)** Dual-path materialization: CI builds canonical SHA-pinned rootfs artifacts from the same recipe, and host trays on macOS + Windows fetch those by default (via the existing `tillandsias-vm-layer::fetch` Windows-added primitive). Local on-host materialization remains the dev/audit path via `--materialize-local`. Linux is materializer-native, fetches nothing. This resolves the chicken-and-egg of needing a Linux build env to build the Linux env on non-Linux hosts.
- **ADDED (D6)** CI publishes both `<arch>.tar` (consumed by WSL `--import` and Linux dev) and `aarch64.img` (raw EFI+ext4 image consumed by macOS Virtualization.framework). The `.img` is produced by `materialize::macos::tar_to_vfr_img` running on the Linux CI runner (the conversion is OS-independent — parted/sgdisk + mkfs.ext4 + copy-in).
- **MODIFIED (D6)** `manifest.toml`'s `[output].expected_rootfs_sha` is now keyed by `<arch>.<format>` (`"aarch64.tar"`, `"aarch64.img"`, `"x86_64.tar"`) so the parity check covers both formats.
...
- **MODIFIED** `openspec/specs/vm-provisioning-lifecycle/spec.md` (delta): replace the binary-download requirements with recipe-materialization requirements; preserve the condensed-status UX contract verbatim; update Cross-references.
...
- **CI**: a new `recipe-smoke` job materializes the recipe on `ubuntu-latest` (x86_64) and `macos-latest` (aarch64) and verifies the resulting rootfs boots, runs `tillandsias-headless --version`, and connects on vsock port 42420. No GitHub-release upload happens for the rootfs itself.
