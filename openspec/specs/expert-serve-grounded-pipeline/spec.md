...
<!-- # freshness: auditor=macos-tlatoanis-macbook-air-claude-20260918t210700z date=2026-09-22 verdict=synced scope=full sync of the expert-serve-grounded-pipeline change tree on its acceptance (920-pxg6 task 4.5 accepted 2026-09-20); requirements are the change deltas unmodified -->
...
The grounded expert pipeline is ONE function with two front-ends. Both the
`expert-serve` HTTP endpoint and the `pipeline` CLI arm obtain their answers by
calling it, so a citation rule, a refusal grammar or a freshness frame cannot
drift between the surface a human uses and the surface an agent uses.

Synced from
`openspec/changes/archive/expert-serve-grounded-pipeline-completed-2026-09-22/`
when that change was accepted. The requirements below are its deltas verbatim; the change tree is
archived alongside this sync so the proposal, design and task record stay
readable without being mistaken for live spec.
