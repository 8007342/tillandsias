...
The status chip SHALL be a single-line, additive string composed of:
...
- **THEN** chip shows `✅ Verifying environment …`
- **AND** as each subsystem builds, its emoji is appended: `✅🧭 Building enclave ...`
- **AND** user sees progress accumulate without menu flicker

#### Scenario: All subsystems online
- **WHEN** all images finish building and start running
- **THEN** chip shows `✅🧭🕸️🛡️🧠🔀🪞🔨 OK`
- **AND** this transition completes in under 30 seconds (typical subsequent starts)

### Requirement: Unhealthy Stage

A new `Stage::Unhealthy` variant signals that at least one subsystem has failed and no concurrent retry is in progress. When detected:

1. The entire menu collapses to a single item: `🥀 Unhealthy environment`
2. The subsystem divider, sign-in options, project list, and other menu items are hidden
3. A signature line (`version` / `— by Tlatoāni` / `Quit`) remains below the unhealthy indicator
4. Detail of which subsystem failed is ONLY in the log (per "single line, detail in logs" preference)

#### Scenario: Forge build fails
- **WHEN** forge container build encounters an error
- **THEN** menu transitions to `🥀 Unhealthy environment` / divider / signature / Quit
- **AND** no retry is in flight
- **AND** detailed error is in the log (user can inspect with --log-forge or UI log viewer)

### Requirement: Cold-Start Baseline

When the tray starts and NO subsystem has yet reported a build status, AND the stage is `Booting`, the chip SHALL read:
...
- **THEN** immediately shows `✅ Verifying environment …`
- **AND** user sees the tray is responsive (not hung or broken)

### Requirement: Locale Keys for Chip

The tray SHALL add the following locale keys (en, de, es):
...
Locale-parity tests (`every_en_key_exists_in_es`, `every_en_key_exists_in_de`) SHALL enforce that all three locales have entries for every key.

### Requirement: Sign-In Label Rename

The "Sign in to GitHub" menu item SHALL be renamed:
