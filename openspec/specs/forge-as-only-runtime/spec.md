...
enter container argv or mounts. The idiomatic Podman layer MUST capture stdout
produced by that invocation and return its real process exit status while the
existing TUI API continues to inherit stdout.
