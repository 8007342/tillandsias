# Traces for versioning

Code implementing this spec (auto-generated — do not edit).
Run `./scripts/generate-traces.sh` to regenerate.

## Annotated locations

- [images/chromium/Containerfile.core#L1](../../../images/chromium/Containerfile.core#L1)
- [images/chromium/Containerfile.framework#L13](../../../images/chromium/Containerfile.framework#L13)
- [scripts/bump-version.sh#L21](../../../scripts/bump-version.sh#L21)
- [scripts/hooks/pre-push-local-gate.sh#L3](../../../scripts/hooks/pre-push-local-gate.sh#L3)
- [scripts/hooks/pre-push-version-guard.sh#L3](../../../scripts/hooks/pre-push-version-guard.sh#L3)
- [scripts/install-hooks.sh#L3](../../../scripts/install-hooks.sh#L3)
- [scripts/local-ci.sh#L3](../../../scripts/local-ci.sh#L3)
- [scripts/release-preflight.sh#L3](../../../scripts/release-preflight.sh#L3)
- [scripts/update-convergence-dashboard.sh#L2](../../../scripts/update-convergence-dashboard.sh#L2)
- [scripts/verify-version-monotonic.sh#L3](../../../scripts/verify-version-monotonic.sh#L3)
