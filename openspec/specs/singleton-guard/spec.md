...
- **THEN** a lock file MUST be created with the current PID and the application MUST start normally
...
- **THEN** the new instance MUST exit immediately with exit code 0 and MUST log a message indicating an existing instance is running
...
- **THEN** the stale lock file MUST be replaced with the current PID and the application MUST start normally
...
- **THEN** the lock file MUST be created at `$XDG_RUNTIME_DIR/tillandsias.lock`

#### Scenario: Linux without XDG_RUNTIME_DIR
- **WHEN** the app starts on Linux and `$XDG_RUNTIME_DIR` is not set
- **THEN** the lock file MUST be created at `/tmp/tillandsias-<uid>.lock`

#### Scenario: macOS
- **WHEN** the app starts on macOS
- **THEN** the lock file MUST be created at `$TMPDIR/tillandsias.lock`
...
- **THEN** the lock file MUST be removed before the process exits
...
- **THEN** the lock MUST be considered valid and the new instance MUST exit
...
- **THEN** the lock MUST be considered stale and the new instance MUST take over
