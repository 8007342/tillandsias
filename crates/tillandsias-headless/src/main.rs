...
fn build_opencode_forge_args(
    ...
    if delegated_json_requested(prompt) {
        args.extend([
            "--env".into(),
...
async fn run_agent_container_attached(
    ...
    eprintln!("[forge-result] {stage} {}", report.outcome.summary());
    for error in &report.infrastructure_errors {
        eprintln!("[forge-result] {stage} FAILED: {error}");
    }
    if report.outcome.is_success() && report.infrastructure_errors.is_empty() {
        ...
        let mut reason = report.outcome.summary();
...
"#,
        ...
        let nonzero = runtime
            ...
            .expect("nonzero current run");
        match nonzero.outcome {
            crate::agent_result::AgentOutcome::Failed { reason } => {
                assert!(reason.contains("37"), "{reason}")
            }
            other => panic!("real exit 37 must fail, got {other:?}"),
        }
