...
## Forge Awareness & Ephemeral Architecture

- **Fast bootstrap**: Invoke the `/forge-quick-intro` skill immediately upon
  starting a session in a forge environment to quickly bootstrap awareness of
  pre-installed tools and runtimes with minimal token overhead.
- **Fully ephemeral workspace**: Workspaces run exclusively inside the forge
  container under `/home/forge/src/<project>` backed by an ephemeral RAM tmpfs
  (`mode=0777`). Host disks are NEVER touched (`HOST/src/<anything>` is never
  created or modified) — the pipeline is strictly:
  `CLOUD > PROJECT > AGENT -> GIT_MIRROR > FORGE`.
- **Pre-installed tooling**: Rust, Cargo, Java (OpenJDK), Python 3, Node.js, Go,
  Dart, Flutter, Chrome (headless/headful), git, gh, ripgrep, jq, yq, and more
  are pre-installed on `$PATH`. Do not waste tokens searching the disk for
  compilers or attempting to re-install packages.
