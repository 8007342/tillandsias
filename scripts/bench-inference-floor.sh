...
_bif_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"
while [ -n "$_bif_dir" ] && [ "$_bif_dir" != "/" ] && [ ! -f "$_bif_dir/lib/tool-dispatch.sh" ]; do
    _bif_dir="$(dirname "$_bif_dir")"
done
if [ -f "$_bif_dir/lib/tool-dispatch.sh" ]; then
    . "$_bif_dir/lib/tool-dispatch.sh" 2>/dev/null || true
    . "$_bif_dir/lib/tool-materialize.sh" 2>/dev/null || true
fi
