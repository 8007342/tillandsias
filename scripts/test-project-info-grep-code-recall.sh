...
if printf '%s\n' "$out" | grep -q 'showing 50 of 260 matches'; then ok "H1 all 260 files searched, truncation announced"
...
if printf '%s\n' "$out" | grep -q 'alpha.txt:1:only alpha here'; then ok "H2 a|b alternation matches (extended regex)"
...
if printf '%s\n' "$out" | grep -q 'alpha.txt'; then bad "H3 stale result: second call returned the first call's hits"
elif printf '%s\n' "$out" | grep -q "no file matches include"; then ok "H3 empty file set named, no stale result"
...
if printf '%s\n' "$out" | grep -q 'invalid extended regex'; then ok "H4 invalid pattern named"
...
if printf '%s\n' "$out" | grep -q '^No matches found'; then ok "H5 absent string -> No matches found"
...
live() { # <label> <pattern> <include> <path>
    local label="$1" want got nwant f bad_f=""
    want="$(truth "$2" "$3" "$4")"
    got="$(gc "$ROOT" "$(args "$2" "$3" "$4")")"
    nwant=$(printf '%s' "$want" | grep -c . || true)
    if [ "$nwant" -eq 0 ]; then
        if printf '%s\n' "$got" | grep -q '^No matches found'; then ok "$label truth=0 -> No matches found"
        else bad "$label truth=0 but answer: $(printf '%s' "$got" | head -1 | cut -c1-120)"; fi
        return
    fi
    if printf '%s\n' "$got" | grep -qE '^(No matches found|grep_code:)'; then
