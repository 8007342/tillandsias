...
out="$(TILLANDSIAS_TIMING_LOG="$T/span.jsonl" bash "$ROOT/scripts/cycle-metrics.sh" --experts-only --no-experts 2>/dev/null | grep '^timing:')"
case "$out" in
...
out="$(TILLANDSIAS_TIMING_LOG="$T/work.jsonl" bash "$ROOT/scripts/cycle-metrics.sh" --experts-only --no-experts 2>/dev/null | grep '^timing:')"
case "$out" in
...
out="$(TILLANDSIAS_TIMING_LOG="$T/legacy.jsonl" bash "$ROOT/scripts/cycle-metrics.sh" --experts-only --no-experts 2>/dev/null | grep '^timing:')"
case "$out" in
