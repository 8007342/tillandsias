...
mk_fixture() { # $1 = indent spaces, $2 = path
    { printf 'packets:\n'
      ...
      printf '      litmus:fixture-closure-is-read\n'
    } > "$2"
}
...
[ "$pre_good"  = "litmus:fixture-closure-is-read" ] || fail "scorable-splitter:arm1:pinned-splitter-should-read-the-indent-6-closure (got [$pre_good])"
[ "$post_bad"  = "litmus:fixture-closure-is-read" ] || fail "scorable-splitter:arm1:indent-4-closure-still-unread-after-fix (got [$post_bad])"
[ "$post_good" = "litmus:fixture-closure-is-read" ] || fail "scorable-splitter:arm1:indent-6-closure-regressed (got [$post_good])"
[ $rc -eq 0 ] && note "ok:scorable-splitter:arm1-fixture:pre-fix=[] post-fix=[litmus:fixture-closure-is-read]"
...
if cmp -s "$tmp/blind-checker.sh" "$CHECKER"; then
    ...
    (
        ...
        # the pre-fix splitter cannot read.
        {
            ...
            printf '    - plan\n'
            printf '    verifiable_closure: |\n'
            printf '      litmus:witness-closure-is-read\n'
...
packets:
  - packet_id: a-row-whose-context-documents-yaml-before-stating-its-closure
    ...
    verifiable_closure: |
      litmus:prose-does-not-eat-the-closure
PROSE
...
if [ "$prose_closure" != "litmus:prose-does-not-eat-the-closure" ]; then
