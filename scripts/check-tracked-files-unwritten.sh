    ...
    verify)
        ...
        if [ "${n:-0}" -gt 0 ]; then
            echo "violation:gate-wrote-tracked-files:$n"
            {
                ...
                echo ""
                echo "  THE WRITER IS ONE OF TWO KINDS AND THE FILE LIST TELLS"
                echo "  YOU WHICH. Order 1230-26sy: this text used to name only"
                echo "  the first, and both hosts that hit it on 2026-09-16 were"
                echo "  the second — one lost a 40-minute gate hunting fixtures"
                echo "  that were innocent."
                echo ""
                echo "  (a) SOMETHING BESIDE THE GATE wrote in this checkout while"
                echo "      it ran: an agent editing or committing, a second land,"
                echo "      an editor saving. Likely if the paths are ones you were"
                echo "      working on. 1063-363b is about the BYTES UNDER"
                echo "      MEASUREMENT CHANGING — a commit is only one way to do"
                echo "      that, and an edit or a staged file does it just as"
                echo "      well. From gate start, nothing tracked changes."
                echo ""
                echo "  (b) A FIXTURE OR GATE STEP escaped its temp dir. Likely if"
                echo "      the paths are ones you never touched. Check"
                echo "      three-argument cp, a \$TMPDIR that was empty so a"
                echo "      relative path resolved into the repo, and any"
                echo "      'cp \$src \$dst' where \$dst was unset."
                echo ""
                echo "  This check cannot tell (a) from (b) — it sees only that"
                echo "  the bytes moved. The file list above is the discriminator."
            } >&2
            exit 1
        fi
