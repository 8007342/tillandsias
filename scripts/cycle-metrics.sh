...
if [ "$_tl_reporting" = 1 ] && [ -z "${TILLANDSIAS_TIMING_LOG:-}" ]; then
    ...
    if [ -n "$_tl_other" ] && [ "$_tl_other" != "$TIMING_LOG" ] && [ -s "$_tl_other" ]; then
        ...
        echo "violation:metrics-log-split:$TIMING_LOG=$_tl_n_here:$_tl_other=$_tl_n_there" >&2
        {
            ...
            echo ""
            echo "  WHAT THE SECOND FILE USUALLY IS, measured on two hosts:"
            echo "  an ARCHIVE plus fixture debris — not a second live writer."
            echo "  /tmp/tillandsias-timing.jsonl may hold this host's"
            echo "  pre-2026-08-26 history, from before .cache/metrics existed,"
            echo "  plus records written by litmus fixtures. The LIVE log is"
            echo "  .cache/metrics. A reader of either one sees a fraction."
            echo "  CONFIRM ON THIS HOST by the changeover TIMESTAMP, not by"
            echo "  record type — on yolanda the last non-fixture /tmp record is"
            echo "  2026-08-26T04:07Z and .cache begins 04:49Z, with nothing"
            echo "  non-fixture in /tmp after it:"
            echo "      tail -1 $_tl_other"
            echo "      head -1 $TIMING_LOG"
            echo ""
            echo "  MOVE IT ASIDE, NEVER DELETE — it may be the only copy of the"
            echo "  early history of this host:"
            echo "      mv $_tl_other $_tl_other.\$(date -u +%Y%m%d).\$(hostname -s)"
            echo "  Concatenating the archive into the live log is a one-time"
            echo "  operator decision, not something this script should do."
            echo ""
            echo "  Then re-run."
