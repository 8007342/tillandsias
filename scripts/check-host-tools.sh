...
rg|binary|gate|linux|check-ripgrep-available.sh|missing:rg|ORDER 1129-xm5z. check-cheatsheet-refs.sh hard-requires ripgrep (host PATH first, then the builder toolbox, else refuse), and 1087-h2z9 promoted that check from --ci-full into --check, so it runs on every land on every host. Tagged linux for the same reason as openssl above: on Windows the gate re-execs into the tillandsias-build WSL2 distro and this check reports ok:host-tools:linux. The prover is a PRESENCE probe rather than check-cheatsheet-refs.sh, deliberately: that script writes its rg-absent refusal to STDERR and exits 2, while this fixture's falsification arm reads the prover's last STDOUT line, so it could not be falsified here — and it proves a downstream symptom rather than the tool. MEASURED on esmeraldinha 2026-09-12: with rg absent the land refused with `a cheatsheet reference does not resolve (1087-h2z9)` having examined ZERO references, and with rg present the same step passes in 4.2s resolving 577|the provisioning fix is with-wsl2-builder.sh's dnf init set (1129-xm5z); on a builder distro stood up BEFORE that fix, run `dnf install -y ripgrep` once inside it
