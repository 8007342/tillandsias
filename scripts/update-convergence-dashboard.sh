...
_ucd_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"
while [ -n "$_ucd_dir" ] && [ "$_ucd_dir" != "/" ] && [ ! -f "$_ucd_dir/lib/tool-dispatch.sh" ]; do
    _ucd_dir="$(dirname "$_ucd_dir")"
done
if [ -f "$_ucd_dir/lib/tool-dispatch.sh" ]; then
    . "$_ucd_dir/lib/tool-dispatch.sh" 2>/dev/null || true
    . "$_ucd_dir/lib/tool-materialize.sh" 2>/dev/null || true
fi
...
{
    printf '<!-- @trace spec:observability-convergence, spec:knowledge-source-of-truth, gap:OBS-008 -->\n'
    printf '<!-- THIS FILE IS AUTO-GENERATED by scripts/update-convergence-dashboard.sh — DO NOT EDIT. -->\n'
    printf '<!-- Last regenerated: %s at %s UTC -->\n\n' "$generated_date_display" "$generated_time_display"
    printf '# %s\n\n' "$TITLE"
