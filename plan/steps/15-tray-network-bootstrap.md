...
- `openspec/litmus-tests/litmus-tray-network-bootstrap.yaml` asserts
  ...
  all three paths.
- `a24bab17` shipped the final residual: project-container spawn failures now
  classify exit 125 into one typed, actionable diagnostic instead of cascading
  repeated container failures.

## Next action

- Treat Step 15 as closed and move to Step 16: observatorium/OpenCode-web
  readiness should prove the real browser-visible page and attach logs/inspect
  data when it fails.
...
- Checkpoint: router-ordering slices complete (`cf74e176`, `4337f917`,
  `14a8bd77`) and exit-125 cascade UX complete (`a24bab17`)
- Push: no further Step 15 push expected unless the litmus binding generator is
  refreshed

## Handoff note

Step 15 is closed. The next agent should keep the bootstrap path thin and make
Step 16 failures diagnosable from one observatorium or OpenCode-web launch
attempt.

## Repeat-mode progress report shape

- Current phase: completed tray bootstrap hardening
- Focus task: observatorium/OpenCode-web readiness probes
- Blockers: none recorded
- Next action: Step 16 OpenCode-web readiness parity
