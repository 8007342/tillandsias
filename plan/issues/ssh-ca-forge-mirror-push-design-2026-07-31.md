...
- **Q3 — CA rotation cadence, and who is allowed to trigger it.** `sshd` can
  ...
  *2026-08-11 — **RULED by The Tlatoāni** (interactive session): the annual
  proposal is REJECTED in favor of something stronger — **epoch-scoped
  ephemeral CAs**. Both CAs are minted inside Vault at stack provision
  (D2 unchanged: `generate_signing_key=true`, never exported, never on a
  file), live exactly one stack/Vault epoch, and are NEVER persisted across
  epochs. There is no rotation schedule because rotation IS the next stack
  launch; there is no cross-epoch trust because every trust consumer
  (TrustedUserCAKeys, principals file, `@cert-authority` known_hosts line)
  is regenerated by the launcher in the same epoch — distribution is atomic
  with minting, which is precisely the precondition that makes ephemeral CAs
  sound rather than an anti-pattern. The KRL is retained for one intra-epoch
  job only: surgically revoking a single lane's certificate without
  relaunching the stack. Implementation rule 451 must enforce: NOTHING may
  cache trust material across epochs. Mesh caveat recorded: order 563
  inherits "the same CA" from §3 — that assumption is now void; cross-node
  trust is the genuine high-distribution-cost case and 563 must design its
  own trust-root lifetime.*
