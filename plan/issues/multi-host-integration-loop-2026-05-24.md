...
cron (job `7ed95aed`, expires after 7 days unless renewed).
...
- Once the user's local methodology edits are committed, manually nudge the
  loop (or wait for the next tick) so `windows-next` Phase 2 lands on
  `linux-next` and gets test coverage on Linux.
