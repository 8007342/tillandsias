...
Operator principle: the forge is an END-USER product — the Claude harness must launch PRE-AUTHED with ZERO manual user steps, exactly like the GitHub token (`--github-login`) and the Codex token. Nothing should ever draw an interactive login box on the happy path.

### PRIMARY-A (THE FIX — transparent, zero manual user steps): Vault-injected `CLAUDE_CODE_OAUTH_TOKEN`

> **Operator correction (2026-07-23):** an earlier draft of this section told the OPERATOR to run `claude setup-token` on their Mac. The operator flagged that as a NON-END-USER action and therefore WRONG — the operator is the DEVELOPER, not the end user; end users of the forge never run CLI commands. The system must capture, store, inject, validate, and refresh the Claude token TRANSPARENTLY, mirroring Codex and GitHub.

Claude Code's headless/CI pre-auth consumes the **`CLAUDE_CODE_OAUTH_TOKEN`** env — a ~1-year token minted by `claude setup-token` (which itself performs a one-time OAuth login). With that env set before launch there is no login flow, no redraw box, and no browser, so the smear cannot occur. Wire it through the EXISTING forge-harness-auth-via-Vault architecture (plan order 112), mirroring the Codex Vault-injection end-to-end (`codex-oauth-vault.sh` / `codex-device-auth.sh` / `entrypoint-forge-codex.sh`) and the `--github-login` → `secret/github/token` → injection pattern:

- **CAPTURE in the forge's OWN login stage** (NOT on the Mac): `run_provider_login` (`main.rs:6860`) already spins up an ephemeral `--rm` login container with a minted Vault AppRole lease + CA bundle and execs `provider-device-auth claude` (spec `main.rs:6771-6778`; tray gate `provider_auth_satisfied` `:10985` / `ensure_provider_auth` `:11000` runs it ONLY when Vault has no usable credential). Extend that lane to run `claude setup-token` with `BROWSER=/dev/null` and capture the printed token — mirror `codex-device-auth.sh:24-40`. The host never sees the value (stdin-only Vault write, `provider-device-auth.sh:87-88`).
- **STORE in Vault**: a `secret/claude/oauth-token` field (or a new field on `secret/claude/oauth`), parallel to `secret/github/token` (`main.rs:6692`).
- **INJECT at launch**: `entrypoint-forge-claude.sh:79-84` already restores the Claude credential from Vault via `provider-oauth-vault restore`. Extend that helper's claude branch (`provider-oauth-vault.sh:22-26`) to also emit `export CLAUDE_CODE_OAUTH_TOKEN=…` (mirror the `ANTIGRAVITY_TOKEN` env-file pattern `:60-63`), eval it in the entrypoint, and `export BROWSER=/dev/null` before the launch (`:126-139`).
- **VALIDATE + REFRESH before the ~1-year expiry**: the `codex-oauth-session` watcher (`:139`) already harvests credential-FILE refresh-token rotations back to Vault. The env-token path adds a launch-time staleness check that re-mints the setup-token before expiry — the only recurring interactive step, at most once a year, still no browser.

Filed as a READY implementation packet: `plan/issues/forge-claude-transparent-oauth-token-vault-inject-2026-07-23.md`.

### PRIMARY-B (one-time interactive fallback ONLY — no token in Vault yet): plain static URL + no browser
...
- `plan/issues/forge-claude-transparent-oauth-token-vault-inject-2026-07-23.md` — the READY implementation packet for PRIMARY-A (transparent capture → Vault → inject `CLAUDE_CODE_OAUTH_TOKEN` → refresh).
