...
Status: **OPEN** as of 2026-05-26T15:29Z. Windows w1, w2, w3, w4, w6
...
unmerged Windows delta and is 6 commits behind `linux-next` `aa8fc2b9`
(pty_handler AsyncFd and pump-cancel work plus coordination checkpoints).
...
- `w7/recipe-diagnostics-and-branch-sync` remains ready as a no-VM fallback:
  ...
  `linux-next` `aa8fc2b9`. The next useful packet is a Windows branch-sync
...
- summary: >
    ...
    absorb `linux-next` `aa8fc2b9`. Report whether the script still accurately distinguishes the
...
- gated_on:
  - linux deliverable `l9/recipe-artifact-url-and-publish-smoke` (first green
    recipe-publish rootfs `.tar` artifacts and SHA pins)
