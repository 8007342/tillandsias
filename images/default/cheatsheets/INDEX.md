...
- flutter.md                         — building Flutter apps in the forge — web + Linux desktop only (no Android, iO… [pull-on-demand: stub]
...
- gradle.md                          — Java/Kotlin builds with `build.gradle` / `build.gradle.kts` — Android exclude… [pull-on-demand: stub]
...
- nix-flake-basics.md                — You want **reproducible** development environments — same toolchain versions … [pull-on-demand: stub]
...
- npm.md                             — managing JavaScript / TypeScript packages — installing deps, running scripts,… [bundled, partial-verify]
- pip.md                             — installing Python packages — but in the forge prefer per-project venvs (PEP 6… [pull-on-demand: stub]
...
- poetry.md                          — project-style Python with strict dep management — alternative to `pip + venv`… [pull-on-demand: stub]
...
- bash.md [DRAFT]                  — writing shell scripts in the forge — entrypoints, build scripts, glue between… [pull-on-demand: stub]
...
- json.md                          — producing or consuming JSON in the forge — config files, API payloads, jq tar… [bundled, partial-verify]
...
- xml.md                           — parsing or producing XML — `pom.xml`, SVG, RSS/Atom, Android manifests, legac… [bundled, partial-verify]
...
- codex-agent-entrypoints.md                — Launching Codex, Claude, OpenCode, or the maintenance shell from [bundled, partial-verify]
...
- enclave-startup-sequencing.md [DRAFT]     — Starting the Tillandsias enclave (proxy → git → forge → inference), manag… [bundled, partial-verify]
...
- portable-executable-transparent-mode.md   — - https://github.com/tauri-apps/tauri — Reference for multi-mode app architec… [bundled, partial-verify]
...
- socket-container-orchestration.md [DRAFT] — Orchestrating Tillandsias enclave startup (proxy → git → inference → forg… [bundled, partial-verify]
...
- vsock-transport.md [DRAFT]                — wiring the Windows/macOS host shell to the in-VM `tillandsias-headless`, debugg… [bundled, partial-verify]
- vz-framework-provisioning.md [DRAFT]      — building the macOS arm of the Tillandsias VM lifecycle, debugging VZ boot failu… [bundled, partial-verify]
...
- windows-tray-dev.md                       — developing the `tillandsias-windows-tray` thin tray (the [bundled, partial-verify]
- windows-tray-diagnostics.md               — an installed Tillandsias tray on a Windows host is misbehaving and you need to … [bundled, partial-verify]
...
- wsl2-provisioning.md [DRAFT]              — the Windows tray's first-run lifecycle is being implemented or debugged, when r… [bundled, partial-verify]
- wslg-chromium-passthrough.md [DRAFT]      — surfacing the in-VM Chromium container as a window on the Windows desktop, debu… [bundled, partial-verify]
...
- junit.md                         — testing Java code — unit tests through Surefire, integration tests through Fa… [pull-on-demand: stub]
- node-test.md [DRAFT]             — testing JavaScript or TypeScript projects with Node's built-in `node:test` runn… [pull-on-demand: stub]
- playwright.md                    — modern E2E browser testing — TypeScript/JavaScript primary, Python and Java a… [pull-on-demand: stub]
...
- yq.md                               — querying or updating YAML files in the forge — Kubernetes manifests, GitHub A… [pull-on-demand: stub]
