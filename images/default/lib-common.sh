...
inject_startup_context() {
    ...
    if [[ -n "${TILLANDSIAS_ACCEL_ENVELOPE:-}" ]]; then
        ...
        _accel_envelope="accel_class=unknown accel_gpu=unknown accel_gpu_name=- accel_npu=unknown accel_npu_name=- accel_reason=envelope-not-exported accel_cpu_cores=- accel_ram_gb=- accel_side=unknown-side accel_gpu_path=unknown accel_gpu_engine=unknown accel_mem_model=unknown accel_mem_budget_gb=- accel_prefill_dev=cpu accel_decode_dev=cpu accel_decode_crossover_b=unmeasured"
    fi
...
- **Accelerators** — \`${_accel_envelope}\`
  ...
  - **\`accel_mem_budget_gb\` is the ONE memory budget, and on \`accel_mem_model=unified\` you must never add a GPU pool to it** — there is only one pool and summing double-counts the machine. \`unknown\` means the probe cannot tell an integrated part from a discrete board; treat it as unified for the purpose of not summing.
